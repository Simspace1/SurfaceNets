...
Shader "MyTriplanarShader2" {
    ...
    SubShader {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
            ...
            float2 uvX = abs(IN.worldPos.zy % 1);
            float2 uvY = abs(IN.worldPos.xz % 1);
            float2 uvZ = abs(IN.worldPos.xy % 1);

            float2 TexRatio;
            TexRatio[0] = 1/floor(_TexAmtX);
            TexRatio[1] = 1/floor(_TexAmtY);

            uvX[0] = uvX[0]*TexRatio[0]+IN.uv_MainTex[0];
            uvY[0] = uvY[0]*TexRatio[0]+IN.uv_MainTex[0];
            uvZ[0] = uvZ[0]*TexRatio[0]+IN.uv_MainTex[0];
 
            uvX[1] = uvX[1]*TexRatio[1]+IN.uv_MainTex[1];
            uvY[1] = uvY[1]*TexRatio[1]+IN.uv_MainTex[1];
            uvZ[1] = uvZ[1]*TexRatio[1]+IN.uv_MainTex[1];
